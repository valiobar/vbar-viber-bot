 ...
 *
 * Inbound HTTP adapter for AI carousel generation.
...
import { BuildCarouselUseCaseImpl } from "../../../application/use-cases/BuildCarouselUseCase";
...
export function createCarouselBuilderRouter(logger: Logger): Router {
  const router = Router();
  const config = getAIConfig();

  // Service-token middleware (same contract as /api/keyboard-builder)
  router.use((req: Request, res: Response, next: NextFunction) => {
    if (!config.serviceToken) {
      return res.status(503).json({
        error: {
          code: "CAROUSEL_BUILDER_NOT_CONFIGURED",
    ...
    if (req.header("x-service-token") !== config.serviceToken) {
      return res.status(401).json({
        error: { code: "UNAUTHORIZED", message: "Invalid or missing X-Service-Token" },
      });
    }
    next();
  });
  ...
  const getUseCase = () => {
    ...
    return new BuildCarouselUseCaseImpl(provider, logger);
  };
  ...
  // Error mapping (message-based, same style as keyboardBuilder)
  router.use((err: Error, _req: Request, res: Response, _next: NextFunction) => {
    const message = err.message || "Carousel generation failed";
    const lower = message.toLowerCase();
    if (lower.includes("required") || lower.includes("must be")) {
      return res.status(400).json({
        error: { code: "CAROUSEL_BUILDER_VALIDATION", message },
      });
    }
    if (lower.includes("json")) {
      return res.status(502).json({
        error: {
          code: "CAROUSEL_BUILDER_BAD_AI_OUTPUT",
          message: "AI returned an unusable response, try again",
        },
      });
    }
    logger.error("Carousel generation failed", err);
    res.status(500).json({ error: { code: "CAROUSEL_BUILDER_FAILED", message } });
  });
