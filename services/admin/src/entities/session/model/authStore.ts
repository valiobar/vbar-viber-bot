...
import { registerTokenGetter } from "@/shared";
import type {
  LoginResponse,
 ...
 * Authentication state (user, tokens, isAuthenticated) is persisted to localStorage.
 * Transient states (isLoading, error) are not persisted.
 */
  ...
  persist(
    (set, get) => ({
      ...
      refreshAccessToken: async () => {
        ...
        try {
          ...
          const result: ApiResponse<RefreshTokenResponse> =
            await response.json();
    ...
    }),
    {
      ...
      partialize: (state) => ({
        ...
        // Exclude transient states: isLoading, error
      }),
    }
  )
);
